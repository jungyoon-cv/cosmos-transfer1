...
def parse_arguments():
    ...
    argparser.add_argument('--time-scale', type=float, default=1.0,
                           help='Scale simulation time step (e.g. 2.0 = twice as fast, 0.5 = half speed)')
...
class TaillightApp:
    ...
    def _draw_overlays(self):
        ...
        for (x, y, size_px, idx) in rects:
            ...
            colour = (255, 255, 0) if is_on else (90, 90, 0)   # vivid yellow / dark yellow
    ...
    def _save_worker(in_q, width, height, out_root):
        ...
        while True:
            ...
            # ---------- Tail-Light Control MAp ----------
            ctrl_arr = np.zeros((height, width, 3), dtype=np.uint8)
            ...
            for (x, y, size_px, idx) in rects:
                ...
                if idx == 0 and left_on:          # left indicator → G 채널
                    ctrl_arr[y0:y1, x0:x1, 1] = 255
                if idx == 1 and right_on:         # right indicator → B 채널
                    ctrl_arr[y0:y1, x0:x1, 2] = 255
        
            ctrl_img = Image.fromarray(ctrl_arr, mode='RGB')
            ctrl_img.save(os.path.join(out_root, 'control', mask_fname), format='PNG', compress_level=3)
            ...
            for x, y, size_px, idx in rects:
                ...
                colour = (255, 255, 0) if is_on else (90, 90, 0)
                draw.rectangle(
    ...
    def _update_camera_orbit(self, dt):
        # Advance the angle accumulator
        self._orbit_angle += self.orbit_speed * dt        # keep accumulating (no wrap)

        # --- Compute relative camera location according to selected pattern ----
        if self.dot_pattern == "equator":
            ...
            rel_z = self.orbit_center_offset.z + self.orbit_radius * self.orbit_scale_z * math.sin(phi)
...
def main():
    ...
    try:
        while True:
            target_loop_rate = max(1, int(args.fps * args.time_scale))
            app.clock.tick_busy_loop(target_loop_rate)
            dt_sec = app.clock.get_time() / 1000.0
            app._update_camera_orbit(dt_sec)
            app._update_blinker()
